...
async function handleSignup() {
    ...
    try {
        ...
        let data = { fullName: document.getElementById('reg-name').value, role: role, email: email, approved: false, uid: cred.user.uid, school: document.getElementById('reg-school').value };
        if (role === 'student') { 
...
function setupUserDashboard(user) {
    ...
    subs.forEach(s => {
        ...
        div.innerHTML = `<div class="w-14 h-14 bg-indigo-50 text-indigo-600 rounded-full flex items-center justify-center mb-2"><i class="fas fa-book-open"></i></div><span class="font-bold text-xs text-slate-700">${s}</span>`;
...
function openSubject(name) {
    ...
    if (currentUser.role === 'teacher') {
        ...
        document.getElementById('modal-subtitle').innerText = "Quiz Module";
        // Logic for fetching questions based on user.grade will go here
    }
}
...
async function saveQuestion() {
    ...
    const data = {
        ...
        targetGrade: targetGrade, // Specific class (e.g., 5th)
    ...
    if (!targetGrade || !data.question || !data.answer) return alert("Select Class and fill all fields");
    try {
